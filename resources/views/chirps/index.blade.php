<!-- x-app Son componentes propios de blade. Si encerramos nuestras vistas con ellos, podremos aplicar el mismo estilo a todo el sitio. -->
@if(session('status'))
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <!-- @dump($errors)->get('message') Entramos en los errores y solamente obtenemos el error relacionado al 'message' -->
                    <form method="POST" action="{{ route('chirps.store') }}">
                        @csrf
                        <!-- Con old('message') preguntamos si existia un mensaje previamente. Y solo sera enviado si el formulario es correcto. -->
                        <textarea placeholder="Escribe tu mensaje..." name="message" id="" class="block w-full rounded-md border-gray-300 bg-white shadow-sm transition-colors duration-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-indigo-300 dark:focus:ring dark:focus:ring-indigo-200 dark:focus:ring-opacity-50">{{ old('message') }}</textarea>
                        <x-input-error :messages="$errors->get('message')" class="mt-2"/> <!-- Mediante los 'errors' de php, obtenemos el mensaje con el error. -->
